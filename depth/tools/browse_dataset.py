...
from depth.apis.test import replace_str
from depth.datasets.builder import build_dataset
...
def imshow_semantic(img,
    ...
    if out_file is not None:
        # depth = colorize(depth, vmin=min_depth, vmax=max_depth, cmap=cmap)
        mmcv.imwrite(img.squeeze(), out_file)
...
def main():
    ...
    for item in dataset:
        filename = os.path.join(args.output_dir,
                                replace_str(item['ori_filename'])
                                ) if args.output_dir is not None else None
        # expand item['depth_gt'] to 3 channels
        item['depth_gt'] = np.expand_dims(item['depth_gt'], axis=0)
        imshow_semantic(
            item['img'],
            item['depth_gt'],
